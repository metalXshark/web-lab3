...
window.drawGraph = function drawGraph(R) {
    ...
    ctx.fillStyle = "gray"; // yellow with 10% opacity
    ctx.beginPath();
    ...
    ctx.fillStyle = "#gray"; // blue with 10% opacity
    ctx.fillRect(width / 2, height / 2 + R * dynamicScalingFactor, - R / 2 * dynamicScalingFactor, - R * dynamicScalingFactor);
    ctx.strokeStyle = "#FFFFFF";
    ctx.strokeRect(width / 2, height / 2 + R  * dynamicScalingFactor, - R / 2 * dynamicScalingFactor, - R * dynamicScalingFactor);

    // Semicircle (lower left)
    ctx.fillStyle = "#gray"; // green with 10% opacity
    ctx.beginPath();
    ctx.arc(width / 2, height / 2, R / 2 * dynamicScalingFactor, Math.PI, -0.5 * Math.PI);
...
for (let i = 0; i < elements.length; i++) {
    elements[i].style.background = 'red'; // Пример изменения стиля, замените на нужный вам стиль
}
